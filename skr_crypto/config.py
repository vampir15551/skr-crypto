...
def require_installed(override: str | os.PathLike[str] | None = None) -> Path:
    """Resolve the install dir and assert the service files are present.

    Raises NotInstalledError otherwise. Used by every command except
    ``install`` and ``doctor`` (the latter inspects state regardless).
    """
    p = install_dir(override)
    if not (p / ".env").exists() or not (p / "app").exists():
