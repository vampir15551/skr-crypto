...
Drives the service's TronClient via a small subprocess helper. We use
``sys.executable`` so the probe runs in the same interpreter that
hosts the CLI (the ``[server]`` extra must be installed for tronpy
to be importable). ``cwd`` is set to the install dir so dotenv loads
the right ``.env``.
"""
...
def cmd(ctx: click.Context, txid: str, as_json: bool) -> None:
    ...
    result = subprocess.run(
        [sys.executable, "-c", _PROBE_SCRIPT, txid],
        cwd=str(install_dir),
        capture_output=True,
        text=True,
        timeout=60,
