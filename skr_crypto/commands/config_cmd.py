...
def validate(ctx: click.Context) -> None:
    """Run the service's own validate_config() against the current .env.
    ...
    if not py.exists():
        raise SkrCryptoError(f"venv python not found at {py}")
    result = subprocess.run(
        [str(py), "-c", "from app.config import validate_config; validate_config()"],
        cwd=str(install_dir),
        check=False,
    )
